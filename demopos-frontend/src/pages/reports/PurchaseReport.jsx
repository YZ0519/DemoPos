...
export default function PurchaseReport() {
  ...
  const canView = perms.includes('reports_purchases')
    ...
    getPurchaseReport(startDate, endDate)
      .then((res) => {
        ...
        setTotals({
          ...
          totalRoundingAdjustment: d.totalRoundingAdjustment ?? 0,
  ...
  const expandableIds = useMemo(
    () => items.filter(i => i.purchaseItems?.length > 0).map(i => i.id),
    [items]
  )
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        {!loading && totals !== null && (
          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">
            {totals.count} {totals.count === 1 ? 'purchase' : 'purchases'}
          </span>
        )}
      </div>
      ...
      <style>{`
        @media print {
          ...
          .pr-table { font-size: 10px !important; }
          .pr-table .pr-chevron { display: none !important; }
          .pr-sub-row { page-break-inside: avoid; }
          .pr-totals { page-break-inside: avoid; }
          .pr-overflow { overflow: visible !important; }
      ...
      {/* Filter bar */}
      <div className="pr-no-print bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 mb-4">
      ...
      {/* Table card */}
      <div className="pr-card bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
        {!loading && items.length > 0 && (
          <div className="pr-no-print flex items-center justify-end gap-2 mb-3">
        ...
        {loading ? (
          ...
          <div className="pr-overflow overflow-x-auto">
            <table className="pr-table w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-slate-700">
                  <th className="pr-chevron w-8 py-3 px-2" />
                  ...
                  <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-slate-400 text-xs uppercase tracking-wide">Tax</th>
                  ...
                  <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-slate-400 text-xs uppercase tracking-wide">Shipping</th>
                  <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-slate-400 text-xs uppercase tracking-wide">Grand Total</th>
              ...
              <tbody>
                {items.length === 0 ? (
                  ...
                  items.map((item, idx) => {
                    ...
                    const hasItems = item.purchaseItems?.length > 0
                    return (
                      <Fragment key={item.id}>
                        ...
                        >
                          <td className="pr-chevron py-3 px-2 text-gray-400 dark:text-slate-500">
                            {hasItems && (
                              isOpen
                                ? <ChevronDown size={14} />
                          ...
                          <td className="py-3 px-4 text-gray-600 dark:text-slate-400">{item.supplierName ?? '—'}</td>
                          <td className="py-3 px-4 text-gray-700 dark:text-slate-300 text-right">{fmt(item.subTotal ?? 0)}</td>
                          <td className="py-3 px-4 text-gray-700 dark:text-slate-300 text-right">{fmt(item.tax ?? 0)}</td>
                          <td className="py-3 px-4 text-gray-700 dark:text-slate-300 text-right">{fmt(item.discount ?? 0)}</td>
                          <td className="py-3 px-4 text-gray-700 dark:text-slate-300 text-right">{fmt(item.shipping ?? 0)}</td>
                          <td className="py-3 px-4 text-gray-700 dark:text-slate-300 text-right font-medium">{fmt(item.grandTotal ?? 0)}</td>
                        ...
                        {isOpen && hasItems && (
                          <tr className="pr-sub-row bg-blue-50/40 dark:bg-slate-700/40">
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="border-b border-gray-200 dark:border-slate-600">
                                    ...
                                    <th className="text-right py-1.5 pr-4 font-medium text-gray-500 dark:text-slate-400">Unit Cost</th>
                                    <th className="text-right py-1.5 font-medium text-gray-500 dark:text-slate-400">Total</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {item.purchaseItems.map((pi, i) => (
                                    <tr key={i} className="border-b border-gray-100 dark:border-slate-600 last:border-0">
                                      <td className="py-1.5 pr-4 text-gray-700 dark:text-slate-300">{pi.productName}</td>
                                      <td className="py-1.5 pr-4 text-gray-500 dark:text-slate-400">{pi.productSku || '—'}</td>
                                      <td className="py-1.5 pr-4 text-gray-700 dark:text-slate-300 text-right">{pi.quantity}</td>
                                      <td className="py-1.5 pr-4 text-gray-700 dark:text-slate-300 text-right">{fmt(pi.unitCost)}</td>
                                      <td className="py-1.5 text-gray-700 dark:text-slate-300 text-right font-medium">{fmt(pi.total)}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                ...
                {totals && items.length > 0 && (
                  <tr className="pr-totals border-t-2 border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 font-semibold">
                    ...
                    <td className="py-3 px-4 text-gray-900 dark:text-slate-100 text-right text-sm">{fmt(totals.totalTax)}</td>
                    <td className="py-3 px-4 text-gray-900 dark:text-slate-100 text-right text-sm">{fmt(totals.totalDiscount)}</td>
                    <td className="py-3 px-4 text-gray-900 dark:text-slate-100 text-right text-sm">{fmt(totals.totalShipping)}</td>
