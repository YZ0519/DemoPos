...
import { Plus, Edit2, Search, Phone, MapPin, Calendar } from 'lucide-react'
...
export default function Suppliers() {
  ...
  const canView   = perms.includes('supplier_view')
  const canCreate = perms.includes('supplier_create')
  const canEdit   = perms.includes('supplier_update')
  const canDelete = perms.includes('supplier_delete')
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    getSuppliers(page, PAGE_SIZE, debouncedSearch)
      .then((res) => {
        if (cancelled) return
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editingItem) {
        const res = await updateSupplier(editingItem.id, payload)
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
        {loading ? (
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {suppliers.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-10 text-center text-gray-400 dark:text-slate-500 text-sm">No suppliers found.</td>
                  </tr>
                ) : (
                  suppliers.map((supplier, index) => {
                    const isProtected = supplier.id === OWN_SUPPLIER_ID
                    return (
                      <tr key={supplier.id} className="border-b border-gray-50 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                        ...
                        <td className="py-3 px-4 text-gray-500 dark:text-slate-400">{supplier.phone || '—'}</td>
                        <td className="py-3 px-4 text-gray-500 max-w-xs truncate">{supplier.address || '—'}</td>
                            ...
                            {canDelete && !isProtected && (
                              confirmingDeleteId === supplier.id ? (
                                <span className="flex items-center gap-2 text-sm">
                                  ...
                                  <button onClick={() => handleDelete(supplier.id)} className="text-red-600 font-medium hover:underline cursor-pointer">Yes</button>
          ...
          <div className="sm:hidden">
            {suppliers.length === 0 ? (
              <p className="py-10 text-center text-gray-400 dark:text-slate-500 text-sm">No suppliers found.</p>
            ) : (
