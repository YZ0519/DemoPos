...
export default function Brands() {
  ...
  const canView   = perms.includes('brand_view')
  const canCreate = perms.includes('brand_create')
  const canUpdate = perms.includes('brand_update')
  const canDelete = perms.includes('brand_delete')
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editingItem) {
        const res = await brandsApi.update(editingItem.id, fd)
        const updated = res.data?.data
        setBrands((prev) => prev.map((b) => (b.id === editingItem.id ? updated : b)))
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Brands</h1>
        {canCreate && (
          <button
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
        {loading ? (
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {brands.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-10 text-center text-gray-400 dark:text-slate-500 text-sm">No brands found.</td>
                  </tr>
                ) : (
                  brands.map((brand, index) => (
                    <tr key={brand.id} className="border-b border-gray-50 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                      <td className="py-3 px-4 text-gray-400 dark:text-slate-500 text-xs">{index + 1}</td>
                      <td className="py-3 px-4">
                        {brand.image ? (
                          <img
                            src={`${MEDIA_HOST}/${brand.image}`}
                            alt={brand.name}
                      ...
                      <td className="py-3 px-4 text-gray-700 dark:text-slate-300 font-medium">{brand.name}</td>
                      <td className="py-3 px-4 text-gray-500 max-w-xs truncate">{brand.description || '—'}</td>
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          brand.status ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                        }`}>
                          ...
                          {canDelete && (
                            confirmingDeleteId === brand.id ? (
                              <span className="flex items-center gap-2 text-sm">
                                ...
                                <button onClick={() => handleDelete(brand.id)} className="text-red-600 font-medium hover:underline cursor-pointer">Yes</button>
          ...
          <div className="sm:hidden">
            {brands.length === 0 ? (
              <p className="py-10 text-center text-gray-400 dark:text-slate-500 text-sm">No brands found.</p>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-gray-900 dark:text-slate-100">{brand.name}</span>
                        ...
                        {brand.description && (
                          <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5 truncate">{brand.description}</p>
                        )}
                      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={submitting}
                />
                <label htmlFor="brand-status" className="text-sm font-medium text-gray-700 dark:text-slate-300">Active</label>
              </div>
